...
**Restraints:** `distanceLockPairs` (each entry: `res1`, `res2`, optional `distanceAngstrom`, optional `springConst`, optional `rigidSpring` defaulting from legacy `restraintGroupRigidSpring` when omitted). Per pair, `rigidSpring: true` uses a high server-side spring constant; `false` uses that row's `springConst`. Manual pair-spring text can still be used. These produce `spring-pair-xyz.dat` in the job directory; the server passes that filename to the start scripts as ``pair_spring``. Optional **fixed wall**, **pair wall**, **fixed spring**, and **nail** tables from the UI are written as `restraint-fixed-wall.dat`, `restraint-pair-wall.dat`, `restraint-fixed-spring.dat`, and `restraint-nail.dat` when the corresponding `enableWallConst` / `enableWallPair` / `enableSpringConst` / `enableNail` flags are set with non-empty text; ``start/web_restraints.py`` merges those paths into ``run_upside.advanced_config`` for ``Single_Replica.py``, ``Replica_Exchange.py``, and ``Pulling_Simulations.py``. Force-sweep sub-jobs copy the same files from the job root into each sub-directory before running pulling.
